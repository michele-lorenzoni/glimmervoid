...
> **Vincolo: il selfhost è mobile.** Il container `glimmervoid` gira sul laptop, che si sposta (casa, hotel, coworking): l'IP di uscita cambia a ogni rete e la sua reputazione va misurata, non assunta. Harness riutilizzabile per qualificare un IP candidato: stampa IP/ASN, riavvia il container (obbligatorio — le sospensioni vivono in RAM, vedi sotto) e interroga ogni engine via bang.

### Misure reali (2026-08-28) — reputazione IP a confronto

| egress | ASN | esito |
|---|---|---|
| IVPN exit Zurigo `185.212.170.142` | `AS9009 M247` (hosting, condiviso) | 📂 **bloccato dalla prima query**: google/ddg/brave/startpage/qwant/mojeek al 100% CAPTCHA-403-429, zero risultati |
| Linea hotel `149.29.29.202` | `AS174 Cogent` (transit business) | 📂 **funziona**: query pulita dopo restart → 8 risultati, target in posizione 1. Google, Brave, Qwant, Bing, Seznam tutti responsivi |

- **Ipotesi smentita dai dati:** avevo assunto che una rete d'albergo fosse CGNAT affollato, quindi cattiva quanto la VPN. Falso: era una linea business su transit, e ha funzionato al primo colpo mentre la VPN falliva sistematicamente. **La reputazione va misurata per IP, non dedotta dal tipo di posto.** Vale in entrambe le direzioni: nessuna rete locale è buona o cattiva a priori.
- **Ma degrada sotto burst.** 📂 Dopo ~15 query automatiche in due minuti, Google è passato a CAPTCHA (sospensione 24h) e Brave a 429 (1h) *sullo stesso IP che aveva appena funzionato*. Confondente non districato: non sappiamo se sia il volume complessivo o il ritmo da bot dell'harness. Conseguenza pratica: **l'harness stesso brucia l'IP che sta testando** — usarlo una volta sola, e comunque riavviare il container dopo. Conseguenza per il design: qualunque IP singolo, per quanto pulito, è una risorsa a consumo; un'istanza usata intensamente ha bisogno di rotazione o di API, non di un IP migliore.
- Le strade con egress **fisso e indipendente dalla posizione** (SOCKS5 su VPS, static residential, istanza intera sul VPS) restano valide, ma **non sono più urgenti**: la rete locale, quando è decente, basta. Da riconsiderare se il pattern d'uso diventa intenso o se ci si trova spesso su reti bruciate.
...
- **Self-hosting su IP residenziale.** Mini-server a casa via DDNS: banda dell'abbonamento internet, IP residenziale "buono" agli occhi degli engine. Operativamente più lavoro (uptime, NAT, dynamic DNS) ma zero costi ricorrenti.
  - ⚠️ **La VPN sull'host annulla il vantaggio.** 📂 Verificato sul selfhost Docker (2026-08-28): host e container `glimmervoid` escono dallo **stesso IP** (`185.212.170.142`), che è 🌐 `AS9009 M247 Europe SRL`, Zurigo — cioè l'exit IVPN, non l'IP di casa. Docker su Windows eredita il default route della TUN WireGuard, quindi **il container non vede mai l'IP residenziale**. Conseguenza: il selfhost eredita una reputazione IP *peggiore* di Render, perché le range M247 sono fra le più aggressivamente filtrate. Il presupposto "selfhost = IP residenziale" va verificato, non assunto: `docker exec <container> wget -qO- https://api.ipify.org` vs `curl https://api.ipify.org` sull'host.
  - **Nessuna VPN consumer è una soluzione, per costruzione.** 🌐 IVPN rifiuta esplicitamente gli IP dedicati: *"No, we do not offer dedicated or static IP addresses for privacy reasons"* ([knowledgebase](https://www.ivpn.net/knowledgebase/general/do-you-offer-dedicated-or-static-ip-addresses/)), puntando sull'*interface crowding* — più utenti possibile sullo stesso IP. È la stessa proprietà che i rate limiter dei motori usano per marcare l'IP: privacy-per-affollamento e "sembrare un singolo umano" sono obiettivi in conflitto diretto. Vale per qualunque VPN no-log a IP condivisi, non per un difetto del provider. Corollario: il vantaggio del VPS SOCKS5 non è che sia "meno datacenter" di una VPN, è che l'IP è **non condiviso**.
  - **Split-tunnel come prerequisito.** Per far coesistere VPN sul browser e IP residenziale per SearXNG serve escludere il traffico Docker dalla VPN (regola di split-tunnelling per app/subnet sul client VPN, oppure route esplicita per la subnet `bridge` 172.17.0.0/16). Da valutare contro il motivo per cui la VPN è accesa: se serve a proteggere *le ricerche*, escluderle è controproducente e la strada giusta resta il proxy dedicato.
  - **Effetto collaterale geografico.** 📂 Con exit svizzero, Bing risponde a `!bi tiramisu recipe` con `bettybossi.ch` / `swissmilk.ch` / `chefkoch.de`: gli engine geolocalizzano sull'IP e localizzano i risultati, degradando le query tecniche in inglese anche quando l'engine *non* è bannato. Da tenere presente nella scelta della country dell'exit node / del VPS.
...
- ⚠️ **Causa verificata di "zero risultati" (2026-08-28): la sospensione, non il ban.** 📂 Test A/B a un minuto di distanza, stesso IP e stessa query: senza restart 0 risultati, dopo `docker restart` 8 risultati con l'articolo target in posizione 1 — e **un solo engine** ha risposto, `google`. 📂 In parallelo, `curl` diretto a `google.com/search` dalla stessa linea → HTTP 200 **senza** CAPTCHA (mentre `html.duckduckgo.com` → 202 + `anomaly`, quello bloccato davvero). Quindi Google era raggiungibile e SearXNG lo stava panchinando per 24h a causa di **una** challenge transitoria. Combinazione letale: `SearxEngineCaptcha: 86400` (📂 `settings.yml.template:66`) + un solo engine `general` realmente funzionante = un CAPTCHA occasionale spegne la ricerca per un giorno intero.
  - **Diagnostica rapida**: pagina vuota → `docker restart glimmervoid` e riprova. Se torna a funzionare era sospensione, non ban.
  - **Da monitorare, non risolto**: `SearxEngineCaptcha` è stato portato a 600 s. Non è misurato se retry più frequenti provochino escalation lato engine (blocchi più lunghi o più duri). Se le sospensioni di Google diventano più frequenti invece che più brevi, rialzare e coprire con altri engine invece che con i retry.
  - **Engine ancora in panchina**: 📂 `seznam` (9 risultati) e `wiby` (2) sono verificati funzionanti su linea Cogent ma restano `disabled: true` per scelta. Riserva se bing/qwant dovessero cadere.
