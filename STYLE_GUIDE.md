...
Sotto al select c'è uno slider **"Contrasto palette"** (`-3…+3`, step 20% per livello → 60% mix max, salvato in `localStorage['palette-contrast']`). Aggiusta in runtime gli accenti `--color-neon-*` e i testi `--color-cust-{default,url,title,description,dim,placeholder}` mescolandoli — via `color-mix(in srgb, …)` inline su `<html>` — verso bianco/nero (alza contrasto) o verso `--color-cust-body` (riduce). La logica vive in `base.html` (`window.__applyPaletteContrast`, definito dopo le stylesheet così `getComputedStyle` legge le var risolte) e viene riapplicata anche al cambio palette per partire dai default freschi.
